-- Fix missing columns and add sample data safely
-- Run this script in your Supabase SQL editor

-- First, let's check what columns exist in the servers table
DO $$
DECLARE
...
-- Now let's check and fix the user_profiles table
DO $$
DECLARE
    column_exists BOOLEAN;
BEGIN
    ...
    ELSE
        RAISE NOTICE 'display_name column already exists in user_profiles table';
    END IF;
    ...
    ELSE
        RAISE NOTICE 'updated_at column already exists in user_profiles table';
    END IF;
END $$;

-- Now let's add sample data only if it doesn't already exist
...
-- Display summary
SELECT 'Database schema updated and sample data added successfully!' as status;
